#!/bin/python3
from core.functions import get_info, path, system, command, get, clear, BeautifulSoup, csv, current_date, zip_longest, document_dir, final, nmap, get_host, check_tor, success, danger, green, red, yellow, green_input, yellow_input, exit, chdir
from core.banner import anonymous_banner, search_banner, check_banner, scan_banner

version = '1.0'


def install():
    if not path.exists('.installed.txt'):
        print("Please run 'sudo bash install.sh' and try again")
        exit()
...
def anonymous():
    while True:
        try:
            anonymous_banner()
            cmd = command('Anonymous Mode')

            if cmd == 1:
                system('sudo torctl start > /dev/null')
                check_tor()
                success('Anonymous Mode Started')
            elif cmd == 2:
                system('sudo torctl stop > /dev/null')
                check_tor()
                danger('Anonymous Mode Stoped')
            elif cmd == 3:
                system('sudo torctl restart > /dev/null')
                check_tor()
                success('Anonymous Mode Restarted')
            elif cmd == 4:
                print(f"Remote IP: {get_info()[1]}")
                final()
            elif cmd == 5:
                system("sudo torctl chngid > /dev/null")
                check_tor()
                success('Anonymous Mode ID Changed')

            elif cmd == 6:
                system('sudo torctl chngmac > /dev/null')
                check_tor()
                success('Anonymous Mode Mac Changed')

            elif cmd == 7:
                system('sudo torctl rvmac > /dev/null')
                check_tor()
                danger('Anonymous Mode Mac Reverted')
...
def search():
    while True:
        try:
            search_banner()
            cmd = command('Search Engines')

            if cmd == 1:
                keywords = green_input('Enter Keywords: ').strip()
                try:
                    result = get(
                        f'https://ahmia.fi/search/?q={keywords}').content
                except:
                    danger('[-] Please check your internet connection')
                    continue

                soup = BeautifulSoup(result, 'html.parser')

                links = soup.select('li.result > h4 > a')
                descriptions = soup.select('li.result > p')
                lastSeens = soup.select('li.result > span.lastSeen')
                ...
                for desc in descriptions:
                    fdesc.append(desc.text.strip())

                for lastSeen in lastSeens:
                    flastSeen.append(lastSeen.text.strip())

                for link in links:
                    url = 'http' + link['href'].strip(
                        f'/search/search/redirect?search_term={keywords}&amp;redirect_url=')
    ...
    Description: {fdesc[i]}
    Url: {furl[i]}
    Last Seen: {flastSeen[i]}

    {'#' * 100}'''))

                saved = yellow_input(
                    'Do you want to save this? ').strip().lower()

                if saved == 'yes' or saved == 'y':

                    file_list = [ftitle, fdesc, furl, flastSeen]
                    exported = zip_longest(*file_list)

                    try:
                        with open(f'{document_dir}/adwt-search-ahmia-{keywords}-{current_date}.csv', 'w') as file:
                            wr = csv.writer(file)
                            wr.writerow(
                                ['Title', 'Description', 'Url', 'Last Seen'])
                            wr.writerows(exported)
                        print(green(
                            f'Saved on path {document_dir}/adwt-search-ahmia-{keywords}-{current_date}.csv'))
                    except:
...
def check_urls():
    while True:
        try:
            check_banner()
            cmd = command('Check Url/s')

            if cmd == 1:
                url = green_input('Enter Url: ').strip().rstrip('\n')
        ...
        Url: {url}
                ...
                file = green_input('File Path: ').strip()

                try:
                    urls = open(file, 'r')
                except:
                    print(red('Please enter a valid path!'))
                    final()
                    continue
                ...
                for url in urls:
                    ...
                    if data != 'error':
                        ...
                        status_code = 'NA'
                        title = 'NA'
                    print(f'{title} : {status} : {status_code} : {url}')
                    ftitle.append(title)
                    fstatus.append(status)
                    fstatus_code.append(status_code)
                    furl.append(url)
                ...
                if saved == 'yes' or saved == 'y':
                    ...
                    file_list = [ftitle, fstatus, fstatus_code, furl]
                    exported = zip_longest(*file_list)

                    try:
                        with open(f'{document_dir}/adwt-check-{current_date}.csv', 'w') as file:
                            wr = csv.writer(file)
                            wr.writerow(
                                ['Title', 'Status', 'Status Code', 'Url'])
                            wr.writerows(exported)
                        print(green(
                            f'Saved on path {document_dir}/adwt-check-{current_date}.csv'))
                    except:
                        print(red('Something wrong...'))
...
def scan():
    while True:
        try:
            scan_banner()
            cmd = command('Scanning')

            if cmd == 1:
                host = green_input('Enter Domain/IP: ').strip()
                # protocol = input('TCP OR UDP: ').strip().lower()
                start_port = int(green_input('Enter Start Port: ').strip())
                end_port = int(green_input('Enter End Port: ').strip())

                ip = get_host(host)

                try:
                    nmap(ip, range(start_port, end_port))
                except:
                    pass
...
def update():
    ...
    print(yellow('[*] Check for Update ...'))
    try:
        result = get(
            'https://github.com/mounibkamhaz/adwt/blob/main/version.txt').content
    except:
        print(red('[-] Please check your internet'))
        final()

    soup = BeautifulSoup(result, 'html.parser')

    v = soup.select_one('#LC1').text.strip()

    if float(v) != float(version):
        chdir('../')
        system('rm -r adwt')
        system('git clone https://github.com/mounibkamhaz/adwt')
        print(green('Anonymous Dark Web Tool Updated Succeffuly.'))
        print(green('Please restart the tool.'))
        final()

    else:
        print(green('You have a latest version.'))
        print('')
        final()
...
def about():
    success(f''' 
          Title:        adwt - Anonymous Dark Web Tool
          Author:       Mounib Kamhaz
          Version:      {version}
          Project:      https://github.com/mounibkamhaz/adwt
